...
import { format, startOfMonth, endOfMonth, isWithinInterval, addDays, isBefore, isAfter } from 'date-fns';
...
export default function Dashboard() {
  ...
  const totalRevenue = monthAppointments.reduce((sum, a) => sum + (a.total_value || 0), 0);
  ...
  const upcomingReturns = patients
    .filter(p => {
      ...
      const returnDate = new Date(p.next_return_date);
      const today = new Date();
      const fifteenDaysFromNow = addDays(today, 15);
      return isWithinInterval(returnDate, { start: today, end: fifteenDaysFromNow });
    })
