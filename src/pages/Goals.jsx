...
export default function Goals() {
  ...
  // Calculate current value for goals
  const calculateCurrentValue = (goal) => {
    ...
    switch (goal.type) {
      case 'Faturamento':
        return monthAppointments.reduce((sum, a) => sum + (a.total_value || 0), 0);
      case 'Pacientes':
        return new Set(monthAppointments.map(a => a.patient_id)).size;
      case 'Procedimentos':
        return monthAppointments.reduce((sum, a) => sum + (a.procedures_performed?.length || 0), 0);
      default:
  ...
  const typeColors = {
    ...
    'Procedimentos': 'bg-purple-100 text-purple-700',
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4 sm:space-y-6">
        {months.map((monthName, monthIndex) => {
          ...
          return (
            <div key={monthIndex}>
              <h3 className="text-xs sm:text-sm font-medium text-stone-500 uppercase tracking-wider mb-2 sm:mb-3">
                {monthName}
              </h3>
                      ...
                      <CardContent className="p-3 sm:p-4">
                        <div className="flex items-start justify-between mb-2 sm:mb-3">
                          <div className="flex items-center gap-2">
                            <div className={`p-1.5 sm:p-2 rounded-lg ${isCompleted ? 'bg-emerald-100' : 'bg-stone-100'}`}>
                              {isCompleted ? (
                                <Check className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-600" />
                              ) : (
                                <Target className="w-3 h-3 sm:w-4 sm:h-4 text-stone-600" />
                              )}
                            </div>
                            <div className="min-w-0">
                              <h4 className="font-medium text-stone-800 text-sm sm:text-base truncate">{goal.title}</h4>
                              <Badge className={`${typeColors[goal.type]} text-[10px] sm:text-xs`}>{goal.type}</Badge>
                            </div>
                          </div>
                          <div className="flex gap-0.5 sm:gap-1 flex-shrink-0">
                            <Button variant="ghost" size="icon" className="h-7 w-7 sm:h-8 sm:w-8" onClick={() => setEditingGoal(goal)}>
                              <Edit2 className="w-3 h-3 sm:w-4 sm:h-4" />
                            </Button>
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-7 w-7 sm:h-8 sm:w-8 text-rose-600 hover:text-rose-700"
                        ...
                        {goal.description && (
                          <p className="text-xs sm:text-sm text-stone-500 mb-2 sm:mb-3 line-clamp-2">{goal.description}</p>
                        )}

                        <div className="space-y-1.5 sm:space-y-2">
                          <div className="flex justify-between text-xs sm:text-sm">
                            <span className="text-stone-500">Progresso</span>
                            <span className={`font-medium ${isCompleted ? 'text-emerald-600' : 'text-stone-700'}`}>
                              {progress.toFixed(0)}%
                            </span>
                          </div>
                          ...
                          <div className="flex justify-between text-[10px] sm:text-xs text-stone-400">
                            <span>
                              {goal.type === 'Faturamento' 
                                ? `R$ ${currentValue.toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`
                                : currentValue
                              }
                            </span>
                            <span>
                              Meta: {goal.type === 'Faturamento' 
                                ? `R$ ${goal.target_value.toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`
                                : goal.target_value
                              }
                            </span>
                          </div>
        ...
        {filteredGoals.length === 0 && (
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-medium text-stone-700 mb-2">Nenhuma meta definida</h3>
            <p className="text-stone-500 mb-4">Crie metas para acompanhar seu progresso</p>
      ...
      <AlertDialog open={!!deleteGoal} onOpenChange={() => setDeleteGoal(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => deleteMutation.mutate(deleteGoal.id)}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input
              ...
              placeholder="Ex: Faturar R$ 50.000"
              required
            />
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder={formData.type === 'Faturamento' ? 'R$ 50000' : '100'}
                required
              />
            </div>
          </div>
