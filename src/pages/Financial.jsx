...
export default function Financial() {
  ...
  const togglePaid = (expense) => {
    updateMutation.mutate({
      id: expense.id,
      data: { ...expense, is_paid: !expense.is_paid, payment_date: !expense.is_paid ? format(new Date(), 'yyyy-MM-dd') : null }
    });
  };
  ...
  const calculateRevenue = () => {
    ...
    filteredAppointments.forEach(a => {
      if (a.payment_method !== 'Cartão Crédito' || !a.installments || a.installments <= 1) {
        revenue += a.final_value || a.total_value || 0;
      }
    });
  ...
  const pendingInstallmentsValue = pendingInstallments.reduce((sum, i) => sum + (i.value || 0), 0);

  const totalRevenue = calculateRevenue();
  const totalExpenses = filteredExpenses.reduce((sum, e) => sum + (e.amount || 0), 0);
  const totalMaterialCost = filteredAppointments.reduce((sum, a) => sum + (a.total_material_cost || 0), 0);
  ...
  const getChartData = () => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      appointments.filter(a => {
        ...
        monthRevenue += a.final_value || a.total_value || 0;
      });
              ...
              <CardContent className="p-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex flex-wrap gap-3 mt-1 text-sm text-stone-500">
                        ...
                        {inst.is_received && inst.received_date && (
                          <span className="text-emerald-600">
                            Recebido: {format(new Date(inst.received_date), 'dd/MM/yyyy')}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <span className="text-lg font-light text-stone-800">
                    R$ {(inst.value || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </span>
                </div>
              </CardContent>
            </Card>
