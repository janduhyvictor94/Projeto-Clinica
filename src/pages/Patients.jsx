                    ...
                    <div className="flex flex-wrap gap-1 sm:gap-2 mt-2">
                      {patient.next_return_date && (
                        <Badge variant="outline" className="text-amber-600 border-amber-200 bg-amber-50 text-[10px] sm:text-xs">
                          ...
                          {safeFormat(patient.next_return_date)}
                        </Badge>
                      )}
                      {Array.isArray(patient.scheduled_returns) && patient.scheduled_returns.slice(0, 2).map((ret, i) => (
  ...
  useEffect(() => {
    if (patient) {
      setFormData({
        ...
        scheduled_returns: Array.isArray(patient.scheduled_returns) ? patient.scheduled_returns : [],
      });
    } else {
      setFormData({
        full_name: '',
