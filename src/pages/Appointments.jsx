...
export default function Appointments() {
        ...
        for (const mat of data.materials_used) {
          ...
          if (material) {
            ...
            await supabase.from('stock_movements').insert({
              ...
              previous_stock: material.stock_quantity, new_stock: newStock, cost_per_unit: mat.cost, total_cost: mat.cost * mat.quantity,
              reason: `Atendimento ${data.patient_name}`, date: new Date()
            });
      ...
      if (data.payment_method === 'Cartão Crédito' && data.installments > 1) {
        ...
        for (let i = 1; i <= data.installments; i++) {
          ...
          installmentsArray.push({
            appointment_id: appointment.id, patient_name: data.patient_name, installment_number: i, total_installments: data.installments,
            value: data.final_value / data.installments, due_date: format(dueDate, 'yyyy-MM-dd'), is_received: true, received_date: format(dueDate, 'yyyy-MM-dd'),
          });
        }
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }) => { const { error } = await supabase.from('appointments').update(data).eq('id', id); if (error) throw error; },
...
function AppointmentModal({ open, onClose, appointment, patients, procedures, materials, onSave, isLoading }) {
  const [formData, setFormData] = useState({
    ...
    notes: '', payment_method: '', discount_percent: 0, installments: 1, procedures_performed: [], materials_used: []
  });
  ...
  useEffect(() => {
    if (appointment) {
      setFormData({
        ...
        date: appointment.date || format(new Date(), 'yyyy-MM-dd'), time: appointment.time || '09:00',
        status: appointment.status || 'Agendado', notes: appointment.notes || '',
        payment_method: appointment.payment_method || '', discount_percent: appointment.discount_percent || 0,
        installments: appointment.installments || 1, procedures_performed: appointment.procedures_performed || [],
        materials_used: appointment.materials_used || []
      });
    } else {
      setFormData({ patient_id: '', patient_name: '', date: format(new Date(), 'yyyy-MM-dd'), time: '09:00', status: 'Agendado', notes: '', payment_method: '', discount_percent: 0, installments: 1, procedures_performed: [], materials_used: [] });
    }
  }, [appointment, open]);
  ...
  const addProcedure = () => {
    ...
    if (proc) {
      setFormData(prev => ({ ...prev, procedures_performed: [...prev.procedures_performed, { procedure_id: proc.id, procedure_name: proc.name, price: proc.default_price }] }));
      setSelectedProc('');
    }
  };
  ...
  const addMaterial = () => {
    ...
    if (mat) {
      setFormData(prev => ({ ...prev, materials_used: [...prev.materials_used, { material_id: mat.id, material_name: mat.name, quantity: parseFloat(matQuantity), cost: mat.cost_per_unit }] }));
      setSelectedMat(''); setMatQuantity(1);
    }
  };
